...
1. `~/.nano/config.json` 中对应 provider 的 `apiKey`
...
系统属性 > 环境变量 > 默认值：`nano.snapshot.enabled`(true) / `nano.snapshot.max`(50) / `nano.snapshot.excludes`(.git,.nano/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `nano.snapshot.dir`(~/.nano/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`NANO_LOG_DIR`(~/.nano/logs) / `NANO_LOG_LEVEL`(INFO) / `NANO_LOG_MAX_HISTORY`(7) / `NANO_LOG_MAX_FILE_SIZE`(10MB) / `NANO_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`nano.react.token.budget`(Integer.MAX_VALUE) / `nano.react.stagnation.window`(3) / `nano.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`nano.llm.connect.timeout.seconds`(60) / `nano.llm.read.timeout.seconds`(300) / `nano.llm.write.timeout.seconds`(60) / `nano.llm.call.timeout.seconds`(600)
...
- shared 模式 close_page 只允许关闭 Nano 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.nano/skills/ < 项目级 .nano/skills/
...
- 环境变量：`NANO_RENDERER=inline|lanterna|plain`
- `NANO_TUI=true`(旧) → lanterna + deprecation 提示
- `NANO_NO_STATUSBAR=true`：禁用底部状态栏
...
- `NANO_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.nano/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.nano/prompts/ < 项目级 .nano/prompts/
...
- editing/safety case 使用固定 fixture 与临时工作区，并增加确定性检查；retrieval/planning 复制当前项目，MCP case 复用已连接 server。
- 批量评测必须显式 `--all`；支持 `--repeat 1-10` 多次采样和 `--fail-under` 交互式门槛提示，报告包含 Trace 汇总指标。
...
# NANO_SNAPSHOT_ENABLED=true
# NANO_SNAPSHOT_MAX=50
# NANO_SNAPSHOT_EXCLUDES=.git,.nano/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# NANO_SNAPSHOT_DIR=/Users/yourname/.nano/snapshots
# NANO_TUI=true
# NO_TUI=true
```
