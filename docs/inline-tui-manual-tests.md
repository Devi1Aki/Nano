...
```bash
cd /Users/yourname/Documents/GitHub/nano
mvn -q clean package -DskipTests
```

产物：`target/nano-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
**预期**：
- Banner 显示简洁的 `Nano v16.1.0` 开屏，不带右侧盒线边框
...
```bash
NANO_RENDERER=lanterna java -jar target/nano-1.0-SNAPSHOT.jar
```
...
**预期**：
- stderr 出现 `⚠️ NANO_TUI=true 已废弃，请改用 NANO_RENDERER=lanterna`
...
```bash
NANO_RENDERER=plain java -jar target/nano-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/nano-1.0-SNAPSHOT.jar
```
...
```bash
NANO_RENDERER=weird java -jar target/nano-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 NANO_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/nano-1.0-SNAPSHOT.jar
```
...
```
*
 Nano  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
**输入**：
```
把 ROADMAP.md 第一行改成 "# Nano 路线图（v16.1 测试）"
```
...
│   [6] 配置文件: ~/.nano/config.json (只读视图，编辑请用编辑器)
...
## 23. NANO_NO_STATUSBAR 禁用状态栏

```bash
NANO_NO_STATUSBAR=true java -jar target/nano-1.0-SNAPSHOT.jar
```
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `NANO_NO_STATUSBAR=true` 即可
