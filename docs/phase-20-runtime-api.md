...
- 生命周期：
  ...
  - `failed`
  - `canceled`
- Worker Pool：默认 2 个后台 worker，可用 `NANO_TASK_WORKERS` 或 `-Dnano.task.workers` 覆盖
...
实现位于 `src/main/java/com/nano/runtime/api/`，使用 JDK 内置 `HttpServer`，不引入 Spring / Javalin。
...
- 仅监听 `127.0.0.1`
- 必须配置 `NANO_RUNTIME_API_KEY` 或 `-Dnano.runtime.api.key`
- 请求头支持：
...
NANO_RUNTIME_API_KEY=test java -jar target/nano-1.0-SNAPSHOT.jar serve --http --port 0
```
