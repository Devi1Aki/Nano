package com.nano.agent;

import com.nano.llm.LlmClient;
import com.nano.llm.LlmTraceLogger;
import com.nano.context.ContextProfile;
import com.nano.context.TokenUsageFormatter;
import com.nano.lsp.LspDiagnosticReport;
import com.nano.memory.ConversationHistoryCompactor;
import com.nano.memory.ExplicitMemoryHints;
import com.nano.memory.MemoryManager;
import com.nano.prompt.PromptAssembler;
import com.nano.prompt.PromptContext;
import com.nano.prompt.PromptMode;
import com.nano.render.PlainRenderer;
...
import com.nano.skill.SkillIndexFormatter;
import com.nano.skill.SkillRegistry;
import com.nano.util.AnsiStyle;
import com.nano.tool.ToolRegistry;
import com.nano.tool.ToolRegistry.ToolExecutionResult;
import com.nano.tool.ToolRegistry.ToolInvocation;
import com.nano.util.TerminalMarkdownRenderer;
import com.nano.image.ImageReferenceParser;
...
public class Agent {
    ...
    public Agent(LlmClient llmClient, ToolRegistry toolRegistry, MemoryManager memoryManager) {
        ...
        this.memoryManager = memoryManager == null ? new MemoryManager(llmClient) : memoryManager;
    ...
    public String getContextStatus() {
        com.nano.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.nano.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.nano.memory.MemoryEntry.estimateTokens(
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.nano.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.nano.memory.MemoryEntry.estimateTokens(
