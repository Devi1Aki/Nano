 ...
 * 1. 系统属性：{@code nano.react.token.budget} / {@code nano.react.stagnation.window} /
 *    {@code nano.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dnano.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dnano.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("nano.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("nano.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("nano.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
