...
import com.nano.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dnano.renderer} > {@code NANO_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code NANO_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        ...
        // 兼容旧 NANO_TUI=true → lanterna
        String legacyTui = System.getenv("NANO_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ NANO_TUI=true 已废弃，请改用 NANO_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 NANO_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
