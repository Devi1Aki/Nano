...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("nano.snapshot.enabled", "NANO_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("nano.snapshot.dir", "NANO_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".nano", "snapshots").toString()));
        int max = readInt("nano.snapshot.max", "NANO_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("nano.snapshot.excludes", "NANO_SNAPSHOT_EXCLUDES", ""));
