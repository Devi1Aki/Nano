...
class TerminalMarkdownRendererTest {
    static {
        System.setProperty("nano.render.color", "false");
    }
    ...
    void fallsBackToKeyValueLayoutForLongTwoColumnTable() {
        String markdown = """
                | 目录名 | 说明 |
                | --- | --- |
                | src/main/java/com/nano | 这里存放 Nano 的主要 Java 源码实现与相关模块 |
                """;

        String rendered = TerminalMarkdownRenderer.render(markdown);

        assertTrue(rendered.contains("目录名 / 说明"));
        assertTrue(rendered.contains("- src/main/java/com/nano"));
        assertTrue(rendered.contains("这里存放 Nano 的主要 Java 源码实现与相关模块"));
    }
}
